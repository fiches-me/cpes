...
Keynes was still influenced by neoclassicals. We still have $\huge{y}\normalsize = C + I$ and $\huge{y}\normalsize = C + S$ (where $S$ = Saving, which empowers $I$ = Investment). But, there is an **inverse causality**.
* For neoclassicals: $\huge{y}\normalsize \rightarrow C \text{ and } I$.
* For Keynes: $C \text{ and } I \rightarrow \huge{y}\normalsize$.

So, there is a choice between Consumption and Savings. Also, for Keynes, $I \neq S$, as we will see in the investment section.

### The propensity to consume

Keynes shows that households have a different **average propensity to consume** (*propension moyenne à consommer*). This can be computed with $c = \frac{C}{\huge{y}\normalsize}$.

The richer you are, the **smaller the part of your revenue consumed**. So, you save more.
The **marginal propensity to consume** is computed as $c_m = \frac{\partial C}{\partial \huge{y}\normalsize}$.
...
Without the State, the equilibrium can be computed with $\huge{y}\normalsize = C + I$, which gives:
...
We are in a similar reflection to Kalecki. The idea is that the stock of capital K is proportional to the level of production ($\huge{y}\normalsize= a \times K$). There is no production output that is not used (every machine is used). 
...
Generally, $0 < v < 1$, but here, to simplify computations, we will take $v = 2$. When $\Delta \huge{y}\normalsize= + 10, \huge{y}_{t+1} - \huge{y}_t = 10$ so $10 \times v = 10 \times 2 = 20$
...
It's computed with $K_t = v \times \huge{y}\normalsize^e_t$, Where $\huge{y}\normalsize^e$ is the **e**xpected production.
