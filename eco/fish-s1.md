---
title: 🐟 Fish éco S1
draft: true
---

# 🐟 Fish — Économie — Semestre 1

## 📚 Définitions

### Investissement

## 👤 Auteurs

> Les auteurs sont rangés par ordre chronologique

### 📜 Intro

#### Aristote (384–322 av. J.-C.)

Il considère l'économie comme **l'administration de la maison** et condamne **l'usure** (l'activité bancaire, prêter de l'argent avec intérêt).

#### St. Thomas d'Aquin (1225–1274)

Il développe l'idée de **juste prix** pour instaurer une **justice de distribution**.

### 💎 Mercantilistes

#### Jean Bodin (1530–1596)

Théorise le lien entre **l'abondance de monnaie** et la hausse des prix (**Inflation**).

#### Thomas Mun (1571–1641)

Dirigeant de la Compagnie des Indes Orientales, il prône une **balance commerciale excédentaire**.

#### Antoine de Montchrestien (1575–1621)

Invente le terme d'**Économie Politique**.

#### William Petty (1623–1687)

*Précurseur de la valeur-travail* et de l'arithmétique politique (**statistiques**).

#### John Locke (1632–1704)

Réflexion sur le taux d'intérêt et la propriété.

### 🍃 Physiocrates

#### François Quesnay (1694–1774)

Médecin de Louis XV, voir [Physiocratie](#Physiocratie)

### ❤ Coeur Classique & Néoclassiques

#### Adam Smith (1723–1790)

Auteur de **Théorie des sentiments moraux** et de **La Richesse des Nations (RDN)**, il s'oppose aux Physiocrates et Mercantilistes avec une richesse provenant **du travail**. Il distingue aussi 3 classes sociales, les *Travailleurs*, les *Capitalistes* et les *Rentiers*. Il développe le concept de **sympathie**, une capacité de se mettre à la place d'autrui, qui modère l'égoïsme. Elle suit la métaphore de la **Main Invisible**, qui dit que le marché s'autorégule par le mécanisme des prix. L'intervention de l'État pourrait **rompre cet équilibre**. L'État doit cependant intervenir dans les dépenses publiques et protéger la population de **la division du travail**.

#### Thomas Malthus (1766–1834)

Craint que l'épargne excessive des rentiers ne réduise la **demande effective**. Il propose des travaux publics pour stimuler l'emploi. Il est surtout connu pour sa **loi de population** : la population augmente de façon **géométrique** (1, 2, 4, 8...) tandis que les ressources alimentaires n'augmentent que de façon **arithmétique** (1, 2, 3, 4...). Ce déséquilibre mène fatalement à la famine sans "obstacles" (guerres, épidémies ou contrainte morale).

#### Jean-Baptiste Say (1767–1832)

Économiste à la source de la loi de Say, qui suppose la **neutralité de la monnaie** et la [Loi des Débouchés](#Loi%20des%20Débouchés)

#### David Ricardo (1772–1823)

Auteur des **principes de  l'économie politique et de l'impôt**. Il est contre les mesure protectionnistes de la Grande Bretagne de son époque car il considère qu'elle vont nuire à l'économie. Il fait une distinction entre la **valeur d'échange** et la **valeur d'usage**. Il propose également la [Théorie de la Rente Différentielle](#Théorie%20de%20la%20Rente%20Différentielle).

#### Sismondi (1773–1842)

Critique l'optimisme libéral. L -> K

#### Karl Marx (1818–1883)

Théoricien du capitalisme qu'il voit comme un système d'exploitation. Il définit la **plus-value** comme la valeur produite par le travailleur mais captée par le capitaliste (travail non payé). Il prédit la **baisse tendancielle du taux de profit** : pour rester compétitifs, les capitalistes remplacent les hommes par des machines (capital constant), mais comme seul le travail humain crée de la valeur, le taux de profit finit par chuter.

#### Léon Walras (1834–1910)

Utilité Cardinale

#### Vilfredo Pareto (1848–1923)

Utilité Ordinale & Ophélimité

### 💣 L'après dépression

#### John Bates Clark (1847–1938)

Développe l'[Accélérateur Keynésien (Clark)](#Accélérateur%20Keynésien%20(Clark))

#### John Maynard Keynes (1883–1946)

**Rupture avec les néoclassiques : la monnaie n'est plus neutre te c'est la demande qui guide l'offre** (C et I -> Y par la [Profitabilité](#Profitabilité)). Il introduit les concepts de [Multiplicateurs Keynésien](#Multiplicateurs%20Keynésien)  et d'[Accélérateur Keynésien](#Accélérateur%20Keynésien). Il insiste sur l'importance des **politiques xxx** (*lien por favor*).

#### John Hicks (1904–1989)

Co-auteur de **la synthèse**, un ouvrage qui réconcilie les travaux néoclassiques et keynésiens. Formalise une partie des concepts de Keynes.

#### Paul Samuelson (1915–2009)

Co-auteur de **la synthèse**, un ouvrage qui réconcilie les travaux néoclassiques et keynésiens.

#### James Tobin (1918–2002)

Propose l'équation du **Q de tobin** qui permet de calculer si un investissement sera profitable ou non.

## 🔎 Notions

> [!WARNING]
> Certaines notions sont peut-être plus des définitions, je fais le tri.

### Mercantilisme

- > **séculariser l'économie**

- > richesse repose sur l'accumulation de **métaux précieux** (*Bullionisme*)

### Physiocratie

- > **Importance de la Nature** : Seule source de produit net.

- > **Classes Sociales** :
    - **Classe Productive** : Fermiers, paysans (créent la richesse).

    - **Classe des Propriétaires** : Propriétaires terriens, Souverain, Église (perçoivent la rente).
    - **Classe Stérile** : Artisans, commerçants (transforment mais ne créent pas).

- > Prônent le "*Laissez faire*" : **Libéralisme**

### Théorie de la Rente Différentielle 

Théorisée par **Ricardo**, elle explique que la rente foncière dépend de la qualité des terres. 
1. On cultive d'abord les terres les plus fertiles. 
2. Avec la croissance démographique, on doit cultiver des terres moins fertiles (terres marginales) où produire coûte plus cher. 
3. Le prix du marché s'aligne sur le coût de production de la terre la **moins fertile**. 
4. Les propriétaires des terres fertiles empochent la différence : c'est la **rente**.

### Loi des Débouchés

**L'offre crée sa propre demande.**

### La Fable du Troc

La monnaie n'est introduite que pour résoudre le problème de la **double coïncidence des besoins**. Elle ne fait que facilité les échanges et **est donc neutre**.

### Théorie Quantitative de la Monnaie 

Toute augmentation de la masse monétaire ($M$) se traduit directement par une augmentation des prix ($P$) selon l'équation $M \times V = P \times T$.

### Utilité Cardinale vs Ordinale

- **Utilité Cardinale** (Jevons, Walras) : On supposait pouvoir mesurer le plaisir (ex: "cette pomme me procure 10 'utils'"). Problème : permet des comparaisons interpersonnelles et donc des arguments pour la redistribution (ce que les libéraux voulaient éviter).
- **Utilité Ordinale** (Pareto) : On ne mesure pas l'utilité, on se contente de **classer** les préférences (ex: "Je préfère A à B"). C'est l'approche retenue par la microéconomie moderne (courbes d'indifférence). 

### Les Lois de Gossen

#### Utilité Marginale Décroissante

L'utilité diminue à chaque unité suplémentaire

#### Égalisation des Utilités Marginales Pondérées

Le consommateur répartie son budget entres différents produits en fonction de l'utilité marginale, pondérée par le prix des produits

### Profitabilité

Concept central chez Keynes et les classiques pour l'investissement. 
- Chez les **Classiques**, elle est menacée par la hausse des salaires et de la rente (état stationnaire). 
- Chez **Keynes**, l'investissement dépend de la comparaison entre l'**efficacité marginale du capital** (rendement escompté) et le **taux d'intérêt** (coût du financement). Si le rendement attendu est supérieur au taux d'intérêt, l'investissement est jugé profitable.

### Propension à consommer

La propension moyenne à consommer se calcule avec $c = \frac{C}{Y}$ et permet de savoir **quel part du revenu est consommé**. 

### Multiplicateurs Keynésien

Constante qui permet de déterminer si une modification de l’investissement/des dépenses de l'état : augmentation du revenu Y est partiellement repensée, ce qui stimule la production, etc.

### Accélérateur (d'investissement) Keynésien (Clark)

Permet de prévoir comment l'investissement va réagir lorsque la production varie (car la demande varie).   

$$
K_t = v \times \large{y}_t \text{  with  } v = \frac{K}{\large{y}}
\Longleftrightarrow
K_{t + 1} - K_t = v(\large{y}_{t + 1} - \large{y}_t) = I_t
$$

### Stabilisateurs Automatiques

Utilisation de **fiscalité proportionnelle** pour amortir les fluctuations d'une économie.

### Politiques conjoncturelles (de court terme)

#### Politiques Monétaires

Modification du taux d’intérêt **de la banque centrale** qui agit sur la quantité de monnaie en circulation dans l'économie. Cela permet aux banques de second rang d'emprunter plus ou moins d'argent et donc de créer plus ou moins de monnaie. 

#### Politiques Budgétaires

Intervention de l'état par l'intermédiaire de son budget - très souvent une **augmentation des dépenses publiques**- mais qui peut aussi **toucher les recettes**.
