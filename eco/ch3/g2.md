...
1. **Closed economy:**
	1.  $\large{y}\normalsize= C + I + G + (X - M)$[^1] (but X and M are ignored, as we are in a closed economy)
	  $\large{y}\normalsize= 1,500 + 4,000 + 2,000 = 7,500$
	2. $\large{y}\normalsize= C + S + T \Longrightarrow T = 1,000$[^2] 
...
2. **Self-financing**
	1. 
	   ...
	   flowchart LR
	    ...
	    F -- W --> H
	   ```
	1. $\large{y}\normalsize= C + I + G = 7,500$ ou $\large{y}\normalsize= W + Div + S_F$ ( donc $S_F = 900$) [^3]
	   **Incoming flux are equals to outgoing flux**
	2. $\pi = Div + S_f = 2,700$
	3. $S = S_H + S_F \Longleftrightarrow S_H = 1,600$
	4. $\frac{S_F}{I} = \frac{900}{2,000 + 900}$ = 31%
	   ➡️ *How much investment is coming from firms?*
1. **Open economy**
	1. $\large{y}\normalsize= C + I + G + (X - M)$ so
	   $M = C + I + G + X - \large{y}\normalsize= 900$
	   $\large{y}\normalsize= C + S + T \Longleftrightarrow S = 3,100$
	   $B_B = G - T = 500$
...
[^2]: Expenses Approach; consommation des ménages

[^3]: Distribution des revenues par les entreprises
