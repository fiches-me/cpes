...
It is noted $\huge{y}\normalsize$ (Yield) or sometimes $\huge{y}$.
...
*In a closed economy without State, $G=0$, $X=0$, $M=0$, so $\huge{y}\normalsize = C + I$.*
...
The demand for money depends on the level of transactions (which depends on GDP $\huge{y}\normalsize$ and Price level $P$).
