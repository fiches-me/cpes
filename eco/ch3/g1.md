...
4. Exchanges with the world (Imports $M$/Exports $X$), also called **trade balance**.
...
As it only mesure the wealth created by the country (and count it only one time), it's broadly a way to compute the GDP
