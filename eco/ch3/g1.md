...
It is noted $\Large{y}\normalsize$ (Yield) or sometimes $\Large{y}$.
...
*In a closed economy without State, $G=0$, $X=0$, $M=0$, so $\Large{y}\normalsize = C + I$.*
...
The demand for money depends on the level of transactions (which depends on GDP $\Large{y}\normalsize$ and Price level $P$).
