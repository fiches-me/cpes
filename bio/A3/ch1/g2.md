...
![](Pasted%20image%2020260505104049.png)
...
Pour étudier les génomes, on peut également utiliser des enzymes de restrictions. Les  enzymes de restrictions coupent l'ADN en deux parties. Or, si on connait la suite d'aa de notre enzyme de restriction (ou l'ADN d'une autre source), alors on peut trouver une séquence. Cela permt également de créer de l'ADN hybride, avec deux bouts collants qui correspondent. On peut alors former des ADN recombinées comme les plasmides des bactéries (mutations/transgenèse) *donc synthétiser de l'insuline de cette manière*. 

Il existe également des **transposant**, partie mobiles d'ADN sur les gènes (partie autre non codante sur le piechart au dessus). Elle peut se déplacer avant ou après un promoteur, inhibant/activant donc celui-ci.
