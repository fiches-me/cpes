...
Par glucose (avec navette malate‑aspartate) :
- Glycolyse : 2 ATP (substrat) + 2 NADH → si MA shuttle : 2 NADH = 5 ATP
- PDH (2 pyruvates → 2 acetyl‑CoA) : 2 NADH = 5 ATP
- Cycle de Krebs (2 acétyl‑CoA) : 6 NADH = 15 ATP ; 2 FADH2 = 3 ATP ; 2 GTP = 2 ATP

Total théorique ≈ 32 ATP par glucose (variation 30–32 selon la navette et l'organisation mitochondriale).

Remarque : les nombres varient selon la source (anciennes valeurs donnaient 36–38 ATP en utilisant P/O = 3/2).
...
## 10. Points pratiques et pièges à connaître

- Palmitate donne ≈ 106 ATP (et non 131) — vérifier méthode de calcul.
- Navettes cytosol↔mitochondrie expliquent différences de rendement (30 vs 32 ATP).
- P/O moderne : NADH ≈ 2.5 ATP ; FADH2 ≈ 1.5 ATP.
- Sources de ROS : Complexe I et III — importance des mécanismes d'antioxydation et des découplants (UCP).

---

## 11. Questions de révision

1. Citer les trois enzymes régulatrices clés de la glycolyse et leurs effecteurs.
2. Écrire l'équation simplifiée catalysée par PDH.
3. Expliquer pourquoi la navette malate‑aspartate donne un rendement ATP différent de la navette glycérol‑3‑phosphate.
4. Calculer rapidement le rendement ATP théorique d'une molécule de glucose (en précisant la navette utilisée).
5. Expliquer l'effet d'un inhibiteur de la chaîne respiratoire sur le bilan énergétique et la production de lactate.

---

## Références et compléments (suggestions)
- Chapitres classiques de biochimie : Lehninger, Berg et al., et Voet & Voet (pour détails mécanistiques et valeurs P/O).
- Articles récents sur la variabilité de la stoichiométrie H+/ATP et implications physiologiques.